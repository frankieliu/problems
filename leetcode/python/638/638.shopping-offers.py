...
#
#
# convert the number of items to single index for dp
def dp(self, num, val):
    """
        num[a]*7^6 + num[b]*7^5 + num[c]*6^3 + num[d]*6^2 + num[e]
    """
    old = 0
    for i in len(0, num):
        old = old * 7 + num[i]
...
class Solution:
    ...
    def shoppingOffers(self, price, special, needs):
        ...
        curridx = cur
        while needs[curridx] == 0:
            curridx = curridx + 1
            needs[curridx] = (needs[curridx] + 1) % 7


    def passThis():
        # Price you would pay without any offers
        best = [x * y for x,y in zip(price, needs)]

        # min price for 3 and 2
        for sp in special:
            if sum([x for x in (needs - sp[0:-1]) if x < 0]) < 0:
                # skip if the special fills more than then needs
                next
            rem = needs - sp[0:-1]
            price = dp(rem) + special[-1]
            if price < best:
                best = price
