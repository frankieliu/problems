...
class Solution:
    ...
    while not done:
      if internal buffer is 'empty'
        read4 to internal buf
      while not done and stuff to read from internal:
        fill out output buffer
        move to next ptr in both
      if internal buffer is fully read
        mark internal buffer as 'empty'

    return output buffer pointer # since this will point to how much filled out
